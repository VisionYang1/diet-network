...
describe('Starting tests for angualr-app', function() {
  ...
  it('navbar-brand should be angualr-app',() => {
    ...
    it('Apples component should be loadable',() => {
      page.navigateTo('/Apples');
      browser.findElement(by.id('assetName'))
      .then((assetName) => {
        return assetName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('Apples');
      });
    });

    it('Apples table should have 5 columns',() => {
      page.navigateTo('/Apples');
      element.all(by.css('.thead-cols th')).then(function(arr) {
    ...
    it('CashToApples component should be loadable',() => {
      page.navigateTo('/CashToApples');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('CashToApples');
      });
    });
